...
nn = model_io.read('../Mitbit_NN_model.txt') # 99.23% prediction accuracy
...
if dataset == "MITBIT":
    ...
    test_X, test_Y = mitbit_test[:,:-1],mitbit_test[:,-1]
...
for i in I[0]:
    ...
    plt.axis('off')
    plt.savefig("../nn_figure/class" + str(cn) + "/" + str(i) + ".png",dpi = 800)
    plt.show(block = True)
